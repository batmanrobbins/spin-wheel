...
var wheelImageName   = "lib/img/prizewheel.png";	// File name of the image for the wheel.
...
var determinedGetUrl = "includes/get_determined_angle.php";  	 // Set to URL of the server-side process to load via ajax when spinMode is determinedAngle or determinedPrize.
/*
...
prizes[0] = {"headline" : "Congratulations","message" : "You won the grand prize!", "startAngle" : 0,   "endAngle" : 39};  // Note how prize end angle is 1 less than start angle of next prize so no overlap.
prizes[1] = {"headline" : "Congratulations","message" : "You received a bonus entry", "startAngle" : 40,  "endAngle" : 79};
prizes[2] = {"headline" : "Sorry","message" : "Try Again", "startAngle" : 80,  "endAngle" : 119};
prizes[3] = {"headline" : "Congratulations","message" : "You won the grand prize!", "startAngle" : 120, "endAngle" : 159};
prizes[4] = {"headline" : "Congratulations","message" : "You received a bonus entry", "startAngle" : 160, "endAngle" : 199};
prizes[5] = {"headline" : "Sorry","message" : "Try Again", "startAngle" : 200, "endAngle" : 239};
prizes[6] = {"headline" : "Congratulations","message" : "You won the grand prize!", "startAngle" : 240, "endAngle" : 279};
prizes[7] = {"headline" : "Congratulations","message" : "You received a bonus entry", "startAngle" : 280, "endAngle" : 319};
prizes[8] = {"headline" : "Sorry","message" : "Try Again", "startAngle" : 320, "endAngle" : 359};
...
function startSpin(determinedValue)
{
	...
	else if (spinMode == "determinedAngle")
	{
		...
		if (typeof(determinedValue) === 'undefined')
		{
			...
			if (determinedGetUrl)
			{
				//PHP for ajax call... commented out so it'll run on github
				//xhr.open('GET', determinedGetUrl, true);
				//xhr.send('');
				//Return angle "100" for demo purposes
				return "100";
	...
	if ((typeof(stopAngle) !== 'undefined') && (wheelState == 'reset') && (power))
	{
		...
		wheelState = 'spinning';
		document.getElementById('spin_button').style.visibility = 'hidden';
		doSpin();
	}
}
	...
	else
	{
		...
		if ((doPrizeDetection) && (prizes))
		{
			...
			for (x = 0; x < (prizes.length); x ++)
			{
				...
				{
					// Do something with the knowlege. 
					$('.olTitle').html(prizes[x]['headline']);
					$('.olMessage').html(prizes[x]['message']);
					$('#resultMsg').modal();
					break;
				}
			}
